        ...
        <form action="{{ route("admin.products.update", [$product->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group {{ $errors->has('category_id') ? 'has-error' : '' }}">
                ...
                <select name="category_id" id="category_id" class="form-control" required>
                    ...
                    <option value="{{ $id }}" {{ (old('category_id') == $id || isset($product) && $product->category->id == $id) ? 'selected' : '' }}>{{ $categories }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('subcategory_id') ? 'has-error' : '' }}">
                ...
                <select name="subcategory_id" id="subcategory_id" class="form-control">
                    ...
                    <option class="temp-subcategory" value="{{ $id }}" {{ (old('subcategory_id') == $id || isset($product) && $product->subcategory->id == $id) ? 'selected' : '' }}>{{ $subcategories }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group {{ $errors->has('featured_image') ? 'has-error' : '' }}">
                ...
                <input type="file" id="featured_image" name="featured_image" class="form-control" value="{{ old('featured_image', isset($product) ? $product->featured_image : '') }}">
            ...
            <div class="form-group {{ $errors->has('long_desc') ? 'has-error' : '' }}">
                ...
                <input type="text" id="long_desc" name="long_desc" class="form-control" value="{{ old('long_desc', isset($product) ? $product->long_desc : '') }}">
            ...
            <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                ...
                <div class="form-check" id="status">
                    <div class="row">
                        <div class="col-sm-2">
                            <input type="checkbox" id="is_featured" name="is_featured" class="form-check-input" value="1" {{ isset($product) && $product->is_featured == 1 ? 'checked' : '' }}>
                            <label for="is_featured" class="form-check-label">Featured</label>
                        </div>
                        <div class="col-sm-2">
                            <input type="checkbox" id="is_latest" name="is_latest" class="form-check-input" value="1" {{ isset($product) && $product->is_latest == 1 ? 'checked' : '' }}>
                            <label for="is_latest" class="form-check-label">Latest</label>
                        </div>
                        <div class="col-sm-2">
                            <input type="checkbox" id="is_active" name="is_active" class="form-check-input" value="1" {{ isset($product) && $product->is_active == 1 ? 'checked' : '' }}>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        // on change category, subcateogory request
        $('#category_id').change(function() {
            // get category id
            var category_id = $(this).val();
            var url = "{{ url('/ajaxSubcategoryFromCategory') }}";
            var csrf = "{{ csrf_token() }}";

            $("#subcategory_id").children('.temp-subcategory').remove();
            ...
            if(category_id) {
                $.ajax({
                    ...
                    success: function(res)
                    {
                        var html = "";
                        $.each(res, function() {
                            html += "<option class='temp-subcategory' value="+this.id+">"+this.name+"</option>";
                        });
